...
A Magic Square is matrix of integers that:
* has square dimensions, *n*×*n*
* contains all the integers from 1 to *n* squared
* has every row, column, and diagonal adding up to the same number

Complete the program in *Main.java* so that the method `isMagicSquare()` returns `true` if `matrix` is a Magic Square and `false` otherwise. 

There are 12 pre-made tests provided but your method should work for other tests too. 

**Hint**: Consider using the equation for the *magic number*.
